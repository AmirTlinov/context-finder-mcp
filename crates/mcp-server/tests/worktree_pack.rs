...
#[tokio::test]
async fn worktree_pack_lists_git_worktrees_and_branch_hints() -> Result<()> {
    ...
    // Avoid branch-name flakiness across git versions/configs.
    ...
    assert!(
        text.contains("worktree_pack") || text.contains("A: worktree_pack"),
